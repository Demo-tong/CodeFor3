#include <unistd.h>
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <time.h>
#include <pwd.h>
#include <grp.h>
#include <dirent.h>
#include <linux/limits.h>
#include <sys/ioctl.h>

#define PARAM_NONE 	0 				//无参数
#define PARAM_A 	1 				//-a; 显示所有文件
#define PARAM_L 	2 				//-l; 一行只显示一个文件的详细信息
#define PARAM_U		4				//-u;
#define PARAM_R		5				//-R;
#define PARAM_F		8				//ls 文件名

int end=0;				
int MAXROWLINE;
int g_leave_len;	 		//一行剩余长度，用于输出对齐
int g_maxlen;				//存放某目录下最长文件名的长度
struct stat buf;			//存储文件信息结构体
int count=0;
int count2=0,count3=0;			//记录非隐藏文件 所有文件的个数

/* 输出错误信息函数 */
void my_err (char *err_string, int line)
{
	fprintf (stderr, "line: %d\n", line);
	perror ("err_string");
	exit (0);
}

//分栏输出
void dis (char *name){
	char name1[256][PATH_MAX + 1];
	int i=0,m=0,n=0,j=0,len=0,num=0;
	if(count3 != count2){
		strcpy(name1[count3],name);
		count3++;
	}
	if(count3 == count2){
		
		int x=(int)(MAXROWLINE/g_maxlen);                       //一行能输出的个数
        	int y=(int)(count2%x);
       		int z=(int)(count2/x);                                  
	        if (y != 0)
        	        z=z+1;						//需要输出几行
		//输出
		for(n=0;n<z;n++){		//行数
			j=n;
			m=0;
			while(m < x )		//输出每一行
			{	
				if(j > count2){			//判断当前下标是否越界，越界则输出空格
					len=g_maxlen;
					for (i = 0; i < len; i++) {
                                                printf (" ");
                                        }
				}
				else{				//下标不越界
					len = strlen (name1[j]);               
		      			int k=len/3;
				        printf ("%-s", name1[j]);//输出
					num++;
				
			        	if(((int) * name1[j]) < 0 )                                 
			                	len=len-k;                                    
					len=g_maxlen-len;
				        for (i = 0; i < len; i++) {
			                	printf (" ");          //用空格补齐，和最长文件名对齐
       					}
				}
				j+=z;		//当前下标加行数
				m++;		//一行输出的个数
			}
			printf("\n");
		}
	}
}

/* 在没有使用-l参数的选项时，打印一个文件名，打印时上下对齐 */
void display_single (char *name)
{
	int i, len;
	//如果本行不足以打印一个文件名则换行
	if (g_leave_len < g_maxlen) {
		printf ("\n");
		g_leave_len = MAXROWLINE;
	}
	len = strlen (name);					//为中文时实际长度为每个中文三个字节
	int x=len/3;						//实际输出占两个字节，每个中文少的一个字节
	len = g_maxlen - len; 					//以最长文件名为标准还剩下多少长度
	printf ("%-s", name);
	int j=0;
	
	if(name[0] == '.'){
		for(i=1;i<strlen(name);i++){
			name[j]=name[i];
			j++;
		}
		if(((int) * name) < 0 )                                
                len=len-2;
	}
		
	
	if(((int) * name) < 0 )					//文件名为中文
		len=len+x;					//将每个中文少的一个字节用空格补齐
	for (i = 0; i < len; i++) {
		printf (" ");					//不足最大长度，输出空格补齐
	}

	g_leave_len -= g_maxlen; 				//更新该行当前剩下的字符数
}


/* 获取文件属性并打印 */
void display_attribute (struct stat buf, char *name)
{
	char 		buf_time[32];
	struct passwd 	*psd; 					//从该结构体中获取文件所有者的用户组
	struct group 	*grp; 					//从该结构体中获取文件所有者所属组的组名

	/* 获取并打印文件类型 */
	if (S_ISLNK (buf.st_mode)) {		//链接文件
		printf ("l");
	}
	else if (S_ISREG (buf.st_mode)) {	//正规文件
		printf ("-");
	}
	else if (S_ISDIR (buf.st_mode)) {	//目录文件
		printf ("d");
	}
	else if (S_ISCHR (buf.st_mode)) {	//字符设备文件
		printf ("c");
	}
	else if (S_ISBLK (buf.st_mode)) {	//块设备文件
		printf ("b");
	}
	else if (S_ISFIFO (buf.st_mode)) {	//有名管道
		printf ("f");
	}
	else if (S_ISSOCK (buf.st_mode)) {	//套接字文件
		printf ("s");
	}
	
	/* 获取并打印文件所有者的权限 */
	if (buf.st_mode & S_IRUSR) {
		printf ("r");
	}
	else 
		printf ("-");

	if (buf.st_mode & S_IWUSR) {
		printf ("w");
	}
	else 
		printf ("-");
	
	if (buf.st_mode & S_IXUSR) {
		if(buf.st_mode & S_ISUID)
			printf("s");
		else
			printf ("x");
	}
	else if( buf.st_mode & S_ISUID )
                printf("S");
	else 
		printf ("-");

    /* 获取并打印与文件所有者同组的用户权限 及特殊权限*/
	if (buf.st_mode & S_IRGRP) {
		printf ("r");
	}
	else 
		printf ("-");

	if (buf.st_mode & S_IWGRP) {
		printf ("w");
	}
	else 
		printf ("-");

	if (buf.st_mode & S_IXGRP) {
		if(buf.st_mode & S_ISGID)
			printf("s");
		else 
			printf ("x");
	}
	else if( buf.st_mode & S_ISGID)
                printf("S");
	else
		printf("-");

	/* 获取并打印其他用户对该文件的操作权限 */
	if (buf.st_mode & S_IROTH) {
		printf ("r");
	}
	else 
		printf ("-");

	if (buf.st_mode & S_IWOTH) {
		printf ("w");
	}
	else 
		printf ("-");

	if (buf.st_mode & S_IXOTH) {
		if(buf.st_mode & S_ISVTX)
			printf("t");
		else
			printf ("x");
	}
	else if( buf.st_mode & S_ISVTX)
                printf("T");
	else
                printf ("-");
	printf ("  "); 							//为了统一对齐而设置的间隔

	/* 根据uid和gid获取文件所有者的用户名和用户组名 */
	psd = getpwuid (buf.st_uid);
	grp = getgrgid (buf.st_gid);

	printf ("%-4d", buf.st_nlink); 					//打印文件的硬链接数

	printf ("%-12s", psd->pw_name);
	printf ("%-12s", grp->gr_name);

	printf ("%6ld", buf.st_size); 					//打印文件的大小

	strcpy (buf_time, ctime (&buf.st_mtime));			//文件最后修改时间
	buf_time[strlen (buf_time) - 1] = '\0'; 			//去掉换行符，用结束标志来替代它

	printf (" %s", buf_time); 					//打印修改时间
}

/*
 * 根据命令行参数和完整路径名显示目标文件
 * 参数flag: 		命令行参数
 * 参数pathname： 	包含了文件的路径名
 */
void display (int flag, char *pathname)
{
	int i, j;
	char name[PATH_MAX + 1];
	struct stat buf;                        //存储文件信息结构体

	/* 从路径中解析出文件名 */
	for (i = 0, j = 0; i < strlen (pathname); i++) {
		if (pathname[i] == '/') {
			j = 0;
			continue;
		}
		name[j++] = pathname[i];
	}
	name[j] = '\0';

	/* lstat函数(获取文件属性)来方便解析链接文件,获取文件的状态信息 */
	if (lstat (pathname, &buf) == -1) {
		printf("error");
		my_err ("lstat", errno);
	}
	/* 
	 * 参数pathname指定文件的路径。
	 * 参数buf表示存储stat结构的缓冲区的地址。
	 * stat结构用于存储文件的状态信息。
	 */
	switch (flag) {
		case PARAM_NONE: 	//没有-l和-a选项
			if (name[0] != '.')
				dis (name);				
			//	display_single(name);
			break;
		case PARAM_A: 		//-a
			display_single (name);
			break;
		case PARAM_L:		//-l
			//printf ("total  %d\n", count);
			if (name[0] != '.') {
				display_attribute (buf, name);		//每个文件的具体信息
				printf ("  %-s\n", name);
			}
			break;
		case PARAM_A + PARAM_L:	//-a -l
			//printf ("total  %d\n", count);
			display_attribute (buf, name);
			printf ("  %-s\n", name);
			break;
		case PARAM_R:
			if (name[0] != '.')
				display_single (name);
		default:
			break;
	}
}


/* 显示当前目录信息函数 */
void display_dir (int flag_param, char *path)
{
	DIR 		*dir; 					//打开目录获得的目录流
	struct dirent 	*ptr; 					//存储目录信息的结构体
	int 		count=0,count1 = 0; 				//统计当前目录下的文件数目
	char 		filenames[256][PATH_MAX + 1],f[256][PATH_MAX + 1],filename[256][PATH_MAX+1];
	char		temp[PATH_MAX + 1], name[PATH_MAX+1] ,pathname[PATH_MAX+1];

	int i, j, k, len,m=0,n=0;
	struct stat buf1,buf2;
	
	//获得当前目录下的文件中最长文件名以及文件数目
	dir = opendir (path); 					//打开目录
	//目录为空，输出错误信息
	if (dir == NULL) {
		my_err ("opendir", errno);
	}
	g_maxlen = 0; 						//g_maxlen初始化
	//目录不为空
	while ((ptr = readdir (dir)) != NULL) { 		//读取目录下信息
		if (g_maxlen < strlen (ptr->d_name)) {
			g_maxlen = strlen (ptr->d_name); 	//改变当前的最大文件长度
		}
		count++; 					//统计当前目录下文件数目
		if(ptr->d_name[0] != '.')
			count2++;
	}

	closedir (dir);//关闭目录

	if (count > 256) {
		printf ("too many files under this dir: %d", errno);
	}


	//获取当前目录下所有的文件名，将他们存储在filenames数组中
	dir = opendir (path);
	if (dir == NULL) {
		my_err ("opendir", errno);
		printf("%s:\n",path);
		printf("\n\n");
		return;
	}
	if(strcmp(path,"./") != 0 && strcmp(path,"../")){
		strcat(path,"/");
	}
	len = strlen (path);
	for (i = 0; i < count; i++) {
		ptr = readdir (dir); 				//获取文件名称
		
		if (ptr == NULL) {
			my_err ("readdir", errno);
		}
		else{
                strcpy(filename[i],ptr->d_name);
		filename[i][strlen (ptr->d_name)] = '\0';
		count1++;

		strcpy (filenames[i], path);
		filenames[i][strlen(path)] = '\0';
		strcat (filenames[i], ptr->d_name);
		filenames[i][len + strlen (ptr->d_name)] = '\0';
		}
	} /* 存储含当前路径的文件名于filenames中 */
	//执行-u命令
	if(flag_param == PARAM_U){
		for (i = 0; i < count - 1; i++) {//控制循环次数
                        for (j = 0; j < count - 1 - i; j++) {
				stat( filenames[j], &buf1 );
				stat( filenames[j+1], &buf2);
                                if ( ((int)buf1.st_atime - (int)buf2.st_atime) < 0) {//按最后访问时间排序
                                        strcpy (temp, filenames[j]);
                                        temp[strlen (filenames[j])] = '\0';

                                        strcpy (filenames[j], filenames[j + 1]);
                                        filenames[j][strlen (filenames[j + 1])] = '\0';

                                        strcpy (filenames[j + 1], temp);
                                        filenames[j + 1][strlen (temp)] = '\0';
                                }
                        }
                }
		for(i = k; k < count - 1; k++){		//解析出文件名
			strcpy(pathname,filenames[k]);
			for (i = 0, j = 0; i < strlen (pathname); i++) {
				if (pathname[i] == '/') {
       		                	j = 0;
        		                continue;
       	        		}
                		name[j++] = pathname[i];
       			}
   			name[j] = '\0';
			if(name[0] != '.')
				display_single(name);		//调用输出
		}
	}
	
	//参数为-R 
	else if(flag_param == PARAM_R){
		printf("%s:\n",path);				//输出当前目录
		g_maxlen=0;
		for(j=0; j<count1; j++){
			if (g_maxlen < strlen(filename[j]) ){
                	        g_maxlen = strlen (filename[j]);        //改变当前的最大文件长度
                	}
		}

		for(i = 0; i < count1 ; i++){	//输出当前目录下的文件名
			if(filename[i][0] != '.')
				display_single (filename[i]);
		}
		printf("\n\n");
		for(i = 0; i < count; i++){
			if (lstat (filenames[i], &buf) == -1) {
				my_err ("lstat", errno);
       	 		}
			if (S_ISDIR (buf.st_mode) && filename[i][0] != '.') {       //目录文件		
				strcat(f[m],filenames[i]);
				m=m+1;
	        	}	
		}
		
		for(i=0;i<m;i++){
			if(m == 1){
				printf("%s:\n",f[i]);
				return;
			}
			display_dir (flag_param, f[i]);
		}
		if(m == 0)
			return ;
			
	}

	//执行ls 文件名
	else if(flag_param == PARAM_F || end == 1){	
		for(j=0; j<count1; j++){
                        if (g_maxlen < strlen(filename[j]) ){
                                g_maxlen = strlen (filename[j]);        //改变当前的最大文件长度
                        }
                }
		for(i=0; i<count1; i++){
			if(end == 1){
				display(flag_param, filenames[i]);
			}
				
			else{
			if(filename[i][0] != '.')
				display_single(filename[i]);
			}
		}
	}

	else{
		/* 冒泡法对文件名进行排序，排序后按照当前顺序存储在数组中 */
		for (i = 0; i < count - 1; i++) {//控制循环次数
			for (j = 0; j < count - 1 - i; j++) {//相邻两个字符串进行比较,是文件名按首字母顺序排列
				if (strcasecmp (filenames[j], filenames[j + 1]) > 0) {
					strcpy (temp, filenames[j]);
					temp[strlen (filenames[j])] = '\0';

					strcpy (filenames[j], filenames[j + 1]);
					filenames[j][strlen (filenames[j + 1])] = '\0';
				
					strcpy (filenames[j + 1], temp);
					filenames[j + 1][strlen (temp)] = '\0';
				}
			}
		}
		//为ls -l时，输出总用量
		if (flag_param & PARAM_L){
			printf ("total  %d\n", count);
		}
		//调用函数显示每个文件的信息
		for (i = 0; i < count; i++) {
			display (flag_param, filenames[i]);
		}
	}
	closedir (dir);
	/* 如果命令行没有-l的选项，则输出一个回车换行 */
	if ((flag_param & PARAM_L) == 0) {
		printf ("\n");
	}
} 



int main(int argc, char *argv[])
{
	int 		i, j, k, l, num; 				    //num统计“-”的个数
	char 		path[PATH_MAX +1];
	char 		param[32]; 			        	//保存命令行参数，目标文件和目录名不在此
	int 		flag_param = PARAM_NONE; 		//参数种类，即是否有-l和-a选项
	struct stat 	buf;
	char		filename[32];
	char            currentpath[32];
	strcpy (filename, "");
	strcpy (currentpath, "");
	//获取屏幕最大长度
	struct winsize size;
        ioctl(STDOUT_FILENO,TIOCGWINSZ,&size);
        MAXROWLINE =size.ws_col;
	g_leave_len = MAXROWLINE;

	//获取命令参数，并将它们存储在param数组中，以及获得'-'的数目
	j = 0;
	l = 0;
	num = 0;	//命令为 ls -
	int num1=0;	//命令为 ls .
	int num2=0;

	for (i = 1; i < argc; i++) {
		if(strcmp(argv[i],"/") == 0){
			strcpy(filename,"/tmp");
                        filename[ strlen(filename) ] = '\0';
			if(i==1)
                                display_dir(PARAM_A,filename);
                        else if(i == 2)
                                display_dir(PARAM_L,filename);
                        return;
		}
		else if(strcmp(argv[i],"/tmp") == 0){
	                strcpy(filename,"/tmp");
        	        filename[ strlen(filename) ] = '\0';
			if(i==1)
	                	display_dir(PARAM_A,filename);
			else if(i == 2)
				display_dir(PARAM_L,filename);
	                return;
        	}
		else if (argv[i][0] == '-') { 			//此时需要存储参数,统计'-'参数的个数
			for (k = 1; k < strlen (argv[i]); k++) {
				param[j++] = argv[i][k];
		 	}
			num++; 					//保存'-'的个数
			num2++;					
		}
		//命令为 ls ./文件名|| ls ../文件名
                else if(argv[i][0] == '.' && (argv[i][1]=='/' || (argv[i][1]=='.' && argv[i][2]=='/'))){
			j=0;
			if(i == 1)
	                        param[0]='f';	
			if(i == 2){
				param[1]='f';
				end=1;
			}
                        for(k = 0; k < strlen (argv[i]); k++){
                                filename[j++] = argv[i][k];
                        }
                        filename[j]='\0';
			num2++;
                }
                //命令为 ls 文件名
                else if(argv[1][0] != '-' && argv[1][0] != '.'){
                        param[0]='f';
                        strcpy(path,"./");
                        strcpy(filename,path);
                        j=strlen(filename);
                        for(k = 0; k < strlen (argv[i]); k++){
                                filename[j++] = argv[i][k];
                        }
                        filename[j]='\0';	
			num2++;
                }
		//命令为 ls .
		else if(argv[1][0] == '.'){
			num1=1;					//命令为 ls .
			for(k = 0; k < strlen (argv[i]); k++){
				param[j++] = argv[i][k];
			}
			if(argv[i][1] == '.')
				num1=2;				//命令为 ls ..
		}
		//命令为 ls -l 文件名
		else{						
			for(k = 0; k < strlen (argv[i]); k++){
				filename[l++] = argv[i][k];
			}
			filename[l]='\0';
		}
	}
	/* 根据param数组中的参数来获得系统中相应的宏表示，将他们用'|'连接起来 */
	for (i = 0; i < num2; i++) {
		if (param[i] == 'a') {
			flag_param |= PARAM_A;
			continue;
		}
		else if (param[i] == 'l') {
			flag_param |= PARAM_L;
			continue;
		}
		else if (param[i] == 'u'){
			flag_param |= PARAM_U;
			continue;
		}
		else if (param[i] == '.'){
			flag_param |= PARAM_NONE;
			continue;
		}
		else if (param[i] == 'R'){
			flag_param |= PARAM_R;
			continue;
		}
		else if (param[i] == 'f'){
			// 显示具体某一文件的文件信息
			if(end == 0)
				display_dir(PARAM_F,filename);
			if(end == 1)
				display_dir(PARAM_L,filename);			
			return;
		}
		else {
			printf ("my_ls: invalid option -%c\n", param[i]);
			exit (1);
		}
	}
	param[j] = '\0';		//为参数字符数组添加结束标志
	/* 如果没有输入具体的文件名，则就是显示当前目录下文件信息 */
	if (num + 1 == argc || num1 == 1) {
		strcpy (path, "./"); 				//将表示当前目录的字符串写入path中
		path[2] = '\0'; 				//为path添加结束标志
		
		//调用函数，显示文件信息
		if(filename[0]=='.')
			display_dir(flag_param,filename);
		else
			display_dir (flag_param, path); 		//将ls参数和显示目录名传过去

		return 0;
	}
	//处理命令ls ..
	else if (num1 == 2){
		strcpy (path,"../");	//路径为上层目录
		path[3] = '\0';
		
		display_dir (flag_param, path);
		return 0;
	}
	//处理命令 ls -l 文件名
	else if( strcmp(filename,"") != 0 ){
		strcpy (path, "./");
		strcat( currentpath,path );

		strcat( currentpath,filename);
		currentpath[2+strlen(filename)] ='\0';
		if (lstat (currentpath, &buf) == -1) {
              		  my_err ("lstat", errno);
		}
		if(S_ISDIR (buf.st_mode) ){	//为目录文件
			display_dir(PARAM_L,currentpath);
		}
		else{
			display_attribute (buf, filename);
			printf ("  %-s\n", filename);
		}

	}

	return EXIT_SUCCESS;
}
