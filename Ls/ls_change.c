...
void display_LS_a(int cntPoint, int cnt){
    ...
    printf("%d %d %d\n",terminalWidth, wordRowNum, wordColNum);
    for(i = 0; i < wordColNum; i++){
        j = i;
        while(j < CNT){
...
void do_ls(int myJudge, char myOrder[]){
   ...
   /*if(myJudge != LS_TMP && myJudge != LS_L){
    ...
    if((dir_ptr = opendir(dirname)) == NULL){//打开目录文件失败
        ...
        if(myJudge != LS_L){
            while((direntp = readdir(dir_ptr)) != NULL){
                if(direntp->d_name[0] == '.'){
                    ...
                    //printf("%s\n", OutputPoint[cntOutputPoint].FileName);
                    cntOutputPoint++;
                }
                else{
        ...
        else if(myJudge == LS_R){
            int i = 0;
           // while(i <= (cntOutput+cntOutputPoint)){
             while(i <= 1){
                if(isadir(OutputPoint[i].FileName) && (OutputPoint[i].FileName != "." || OutputPoint[i].FileName != "..")){
                    do_ls(LS_R, OutputPoint[i].FileName);
                }else if(isadir(Output[i].FileName) && (Output[i].FileName != "." || Output[i].FileName != "..")){
                    do_ls(LS_R, Output[i].FileName);
                }else{
                    do_ls(LS_TMP, "/");
                }
                ++i;
...
int main(int argc, char *argv[]){
    ...
    if(argc == 1){
        ...
        //getopt用来解析命令行选项参数
        while((ord = getopt(argc, argv, ":latuR")) != -1){
            switch(ord){
                case 'a':
                    do_ls(LS_A, "ls-a");
                break;
